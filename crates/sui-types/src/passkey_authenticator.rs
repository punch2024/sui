...
impl AuthenticatorTrait for PasskeyAuthenticator {
    ...
    where
        T: Serialize,
    {
        let client_data_json: PasskeyClientDataJson = serde_json::from_str(&self.client_data)
            .map_err(|_| SuiError::InvalidSignature {
                error: "get client data".to_string(),
            })?;
        ...
        let client_data_hash = Sha256::digest(self.client_data.as_bytes()).digest;
        ...
        match self.user_signature.scheme() {
            SignatureScheme::Secp256r1 => {
                ...
                pk.verify(&message, &normalized_sig)
                    .map_err(|_| SuiError::InvalidSignature {
                        error: "verify failed".to_string(),
                    })
            }
            // todo: think about whether to support ed25519 and secp256k1
