...
use crate::{
    ...
    shared::{
        ide::{AutocompleteInfo, ExpInfo, MacroCallInfo},
...
pub fn program(
    ...
    debug_print!(context.debug.autocomplete_resolution, ("autocomplete info" => context.env.ide_information.autocomplete_info; dbg));
...
fn resolve_exp_dotted(
    ...
    if let Some(loc) = autocomplete_last {
        ...
        result
        // let ty = sp(error_loc, Type_::UnresolvedError);
        // Box::new(T::exp(ty, sp(error_loc, result.exp.value)))
    } else {
        result
    }
}
...
fn ide_report_autocomplete(context: &mut Context, at_loc: &Loc, in_ty: &Type) {
    ...
    if context.env.overlaps_cursor(at_loc) {
        let ty = core::unfold_type(&context.subst, in_ty.clone());
        let Some(tn) = type_to_type_name(context, &ty, *at_loc, "autocompletion".to_string())
        else {
            return;
        };
        ...
        context.env.add_ide_autocomplete_info(*at_loc, info);
    }
}
...
fn method_call(
    ...
    // report autocomplete for the cursor if it's here
    if context.env.ide_mode() {
        edotted.autocomplete_last = Some(method.loc);
    }
